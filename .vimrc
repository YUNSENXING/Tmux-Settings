...
" 在 C++ 文件中设置快捷键 \e 来编译并运行当前 C++ 程序
autocmd FileType cpp nnoremap <buffer> \e :execute 'VtrSendCommandToRunner! cd "' . expand('%:p:h') . '" && g++ -std=c++11 "' . expand('%:t') . '" -o "' . expand('%:r') . '" && ./' . expand('%:r') <CR>
let g:VtrReuseRunnerPane = 1

" 在执行完命令后，自动关闭新创建的 tmux runner pane
autocmd FileType cpp nnoremap <buffer> \e :call RunAndCloseRunner()<CR>

function! RunAndCloseRunner()
  " 执行命令并创建 runner pane
  execute 'VtrSendCommandToRunner! cd "' . expand('%:p:h') . '" && g++ -std=c++11 "' . expand('%:t') . '" -o "' . expand('%:r') . '" && ./' . expand('%:r') . ' && tmux kill-pane' . "\<CR>"
endfunction
...
inoremap " ""<Left>
" 配对括号时光标会自动跳到括号内
autocmd FileType * inoremap <buffer> ( ()<Esc>i
autocmd FileType * inoremap <buffer> { {}<Esc>i
autocmd FileType * inoremap <buffer> [ []<Esc>i
